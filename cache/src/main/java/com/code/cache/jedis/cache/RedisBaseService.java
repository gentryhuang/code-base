...
import com.code.cache.jedis.util.DataProcessUtil;
...
public abstract class RedisBaseService implements ICacheService {
    ...
    @Override
    public Boolean mset(Map<String, String> keyValues, int expireSecond) {
        ...
        try {
            jedis = getResource();
            String[] params = new String[keyValues.size() * 2];
            int count = 0;
            for (Map.Entry<String, String> keyvalue : keyValues.entrySet()) {
                params[count++] = keyvalue.getKey();
                params[count++] = keyvalue.getValue();
            }
            String result = jedis.mset(params);
            if (STATUS_CODE.equals(result)) {
                isSuccess = true;
    ...
    @Override
    public long msetnx(Map<String, String> keyValues, int expireSecond) {
        long result = 0;
        if (null == keyValues || keyValues.isEmpty()) {
            return result;
        }
    ...
    @Override
    public Boolean msetObj(Map<String, Object> keyValues, int expireSecond) {
        Boolean isSuccess = false;
        if (null == keyValues || keyValues.isEmpty()) {
            return isSuccess;
        }
        ...
        try {
            ...
            for (Map.Entry<String, Object> keyvalue : keyValues.entrySet()) {
                params[count++] = keyvalue.getKey().getBytes();
                params[count++] = DataProcessUtil.serialize(keyvalue.getValue());
            }
            String result = jedis.mset(params);
            if (STATUS_CODE.equals(result)) {
                isSuccess = true;
    ...
    @Override
    public long msetObjnx(Map<String, Object> keyValues, int expireSecond) {
        long result = 0;
        if (null == keyValues || keyValues.isEmpty()) {
            return result;
        }
    ...
    @Override
    public Long hsetnx(String key, String field, String value, int expireSecond) {
        Long result = Long.valueOf(0);
        Jedis jedis = getResource();
        try {
            result = jedis.hsetnx(key, field, value);
            if (result == 1 && expireSecond != -1) {
                jedis.expire(key, expireSecond);
            }
        } finally {
            returnResource(jedis);
    ...
    @Override
    public void rpushx(String key, int expireSecond, String... values) {
        if (key == null || key.trim().length() == 0) {
            return;
        }
        Jedis jedis = null;
    ...
    @Override
    public void lpushx(String key, int expireSecond, String... value) {
        if (key == null || key.trim().length() == 0) {
            return;
        }
        Jedis jedis = null;
    ...
    public boolean ltrim(String key, long start, long end) {
        ...
        try {
            ...
            if (STATUS_CODE.equalsIgnoreCase(ret)) {
                result = true;
            }
        } catch (Exception e) {
            logger.error(e.getMessage());
    ...
    @Override
    public long incrBy(String key, long increment, int expireSecond) {
        Long value = null;
        Jedis jedis = getResource();
        try {
            if (expireSecond != -1) {
    ...
    @Override
    public long decrBy(String key, long decrement, int expireSecond) {
        Long value = null;
        Jedis jedis = getResource();
        try {
            if (expireSecond != -1) {
    ...
    public long expire(String key, int expireSecond) {
        ...
        Jedis jedis = getResource();
        try {
            ret = jedis.expire(key.getBytes(), expireSecond);
        } finally {
            returnResource(jedis);
        }
        return ret;
